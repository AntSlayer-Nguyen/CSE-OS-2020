...
void * mem_alloc(unsigned int size,int alloc_choice) {
	...
	struct mem_region * temp = free_regions;
	printf("Free region: ");
	while(temp){
		printf("%4ld|",temp->size);
		temp = temp->next;
	}
	printf("\n");
	void *pointer = NULL;
	switch(alloc_choice){
		case 1: ;
		    pointer = first_fit_allocator(size);
		    break;
		case 2: ;
            pointer = best_fit_allocator(size);
		    break;
		case 3: ;
		    pointer = worst_fit_allocator(size);
		    break;
		default: ;
		    puts("Wrong choice!!");
			return NULL;
	...
	if (pointer != NULL) {
		printf("Thread [ID = %d]: NEED TO Alloc [%4d bytes] : %p-%p\n",gettid(), size, pointer, (char*)pointer + size - 1);
	}else{
		printf("Thread [ID = %d]: NEED TO Alloc [%4d bytes] NULL\n",gettid(),0);
	}
...
void mem_free(void * pointer) {
	...
	if (current_region != NULL) {
		...
		printf("Thread [ID = %d]: NEED TO Free  [%4ld bytes] : %p-%p\n",gettid(), current_region->size, current_region->pointer,
...
void * best_fit_allocator(unsigned int size) {
	...
	struct mem_region * curr = free_regions;
	struct mem_region * found = NULL;
	unsigned int smallest_size = INT_MAX;
	while(curr){
		if(curr->size >= size && curr->size < smallest_size){
